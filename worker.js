...
async function fullScan(ticker) {
  try {
    ...
    return new Response(JSON.stringify({
      ...
      revenueFormatted: totalRevenue?'$'+fmt(totalRevenue,1e9,'B')+' Trailing Twelve Months':'Pre-revenue',
      nextEarnings,
...
async function getCIK(ticker) {
  try {
    const r = await fetch('https://www.sec.gov/files/company_tickers.json', { headers: { 'User-Agent': 'K1LADEX research contact@k1ladex.com' } });
    const data = await r.json();
